...
type View = "Overview" | "Requirements" | "Changes" | "Evidence" | "Git" | "Audit" | "Team";
...
export default function Home() {
  ...
  useEffect(() => {
    setMode(config && localStorage.getItem(`architect-mode:${config.repoPath}`) === "Team" ? "Team" : "Solo");
  }, [config?.repoPath]);
  function changeMode(next: "Solo"|"Team") {
    if (!config) return;
    localStorage.setItem(`architect-mode:${config.repoPath}`, next);
    setMode(next);
    if (next === "Solo" && view === "Team") setView("Overview");
  }
  ...
  async function propose() {
    ...
    try { await jsonFetch("/api/changes", { method:"POST", body:JSON.stringify({ action:"propose", text:idea, teamMode:mode === "Team" }) }); setIdea(""); await refreshContext(); setView("Changes"); }
    catch (cause) { setError(errorText(cause)); }
  ...
  return <main className="app-shell">
    ...
    {view === "Overview" && <div className="view-content overview">
      ...
      {mode === "Team" && <TeamPanel repoPath={config.repoPath} compact onOpen={() => setView("Team")}/>}
    ...
    {view === "Team" && mode === "Team" && <TeamPanel repoPath={config.repoPath}/>}
