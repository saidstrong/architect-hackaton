...
type LiveSnapshot = { workers:LiveWorker[]; architectUrls:string[] };
...
function LivePanel({workerId,team,live,selectedTaskId,onTeamRefresh,onLiveRefresh}:{workerId:WorkerId;team:TeamSnapshot;live:LiveSnapshot|null;selectedTaskId:string;onTeamRefresh:()=>void;onLiveRefresh:()=>void}) {
  ...
  return <section className="mission-card team-live"><div className="card-kicker"><span>OPTIONAL LIVE EXECUTION · WORKER {workerId}</span><span>{worker?.connection==="online"?"● ONLINE":"○ OFFLINE"}</span></div>
    ...
    {token&&<div className="team-alert"><strong>Copy now · token shown once</strong><p>Architect URL (choose one): {live?.architectUrls.length ? live.architectUrls.map(url=><code key={url}>{url}<br/></code>) : <code>http://&lt;Architect LAN IP&gt;:3101</code>}</p><p>Worker: <code>{workerId}</code></p><p>Token: <code>{token}</code></p><p>On the worker device, set ARCHITECT_URL, ARCHITECT_WORKER_ID, ARCHITECT_WORKER_TOKEN, and TARGET_REPO, then run <code>npm run worker</code>. Keep the token private.</p><button className="secondary compact" onClick={()=>setToken("")}>Hide token</button></div>}
